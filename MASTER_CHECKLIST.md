...
> Tracciamento implementazione: [`04_INTELLIGENCE/REGISTRO_AVANZAMENTO.md`](04_INTELLIGENCE/REGISTRO_AVANZAMENTO.md) (registro canonico).
...
- [x] **Infrastruttura & Size:** Azure Blob (LRS) + DVC. Target Definitivo: **1.5 Terabyte (450 ore)**. Pianificazione basata sul **credito Azure di $200** (budget-driven, non time-driven); piano di spesa per task in [`STRATEGIC_INFRASTRUCTURE_AUDIT.md` §7.1](04_INTELLIGENCE/STRATEGIC_INFRASTRUCTURE_AUDIT.md#azure-spend-plan). Archivio permanente post-Azure: HDD fisico 2 TB (~€100–150). Formato del layer Gold: **WebDataset** tar-shard ([`DOSSIER_TECNICO` §9.2](docs/methodology/DOSSIER_TECNICO.md#medallion)).
...
- [x] **[STRP-001] Contratto Dati & Output MIDI:** Risolto (Executive Briefing 2026-05-20). Formato Gold tensor + packaging WebDataset; **MIDI Mapping Table** `GM↔8-bus` bidirezionale; Hi-Hat con testa di apertura **continua** e uscita selezionabile **CC-continuo / Note-discrete**. Artefatto modello: blob pesi cifrato + header metadati, export PyTorch→RTNeural. Esportabilità RTNeural certificata dal Gate L3. Dettaglio in [`MASTER_SCHEDULING.md` §6](04_INTELLIGENCE/MASTER_SCHEDULING.md#tasks) (F0-T2a, F0-T4b, F0-T8). **Spec di dettaglio F0-T2a LOCKED (2026-05-20)** — recipe YAML, layout byte FP16 `flat-25`, DNA-Trace: [`F0-T2a_RECIPE_DATA_CONTRACT_SPEC.md`](docs/methodology/F0-T2a_RECIPE_DATA_CONTRACT_SPEC.md); MIDI Mapping Table versionata in `docs/specs/midi_mapping_table.yaml`. **Spec di dettaglio F0-T8 LOCKED (2026-05-23)** — schema binario `.opna` (magic `OPNA` + header JSON in chiaro + payload AES-256-GCM + GCM auth tag), key derivation HKDF Poisoned, versioning + loader fail-loud: [`F0-T8_MODEL_ARTIFACT_SPEC.md`](docs/methodology/F0-T8_MODEL_ARTIFACT_SPEC.md).
...
- [x] **[STRP-001] Testing & QA (trasversale):** Risolto (Executive Briefing 2026-05-20).
  ...
  Pattern AI-Adversarial QA in [`SUB_AGENT_GOVERNANCE.md` §6](04_INTELLIGENCE/SUB_AGENT_GOVERNANCE.md#ai-adversarial-qa). Nuovi task F0-T9a/b
  (`MASTER_SCHEDULING.md`); l'harness F0-T9b è **gate test-first di F0-T2b/c/d**.
- [x] **Formati di Distribuzione:** v1.0 = **VST3 + AU**. AAX (Pro Tools) rinviato post-v1.0: richiede firma PACE, incompatibile con la filosofia anti-DRM ([`DOSSIER_TECNICO.md` §11](docs/methodology/DOSSIER_TECNICO.md#licensing)).
...
Livelli di maturità progressivi citati nel Tracking Board di [`MASTER_SCHEDULING.md`](04_INTELLIGENCE/MASTER_SCHEDULING.md#tracking-board) e nei documenti marketing. Definizione unica e vincolante:
...
> ⚠️ **Correzione del SESSION_HANDOVER:** la priorità "Setup Azure = PRIORITÀ 1, sblocca L2" è superata. L2 e L3 sono mini-batch **locali** ([§1](#ai-neural), [§6](#gates)): non richiedono Azure. Il render Azure è gated da L2; il training da L3 (vedi [`MASTER_SCHEDULING.md` §4](04_INTELLIGENCE/MASTER_SCHEDULING.md#credit-scale)).
