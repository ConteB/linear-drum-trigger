...
- **Quota state (aggiornato 2026-05-30, CP-1):**
  - ✅ **`Standard DSv3 Family vCPUs` 16 vCPU APPROVATA** (Italy North) → F2-T1 render
    sbloccato lato compute. **Non toccata dal rifiuto A100.**
  - ❌ **`Standard NCADS_A100_v4 Family vCPUs` 24 vCPU DEFINITIVAMENTE NEGATA** (ticket
    Microsoft Support chiuso/respinto, comunicato CEO 2026-05-30). L'A100 non sarà
    disponibile su questa subscription nella finestra del credito.
  - 🔄 **Pivot GPU (Decision Lock CEO 2026-05-30, CP-1):** il modello F2-T3 è una TCN
    da **331k parametri** (mini-L3 traina in 3.2h su Mac M5/MPS) — l'A100 era
    sovradimensionata. Nuova richiesta quota: **`Standard NCASv3_T4 Family vCPUs`
    16 vCPU** (Italy North, stessa region dello storage Gold → no egress) per una VM
    **`Standard_NC16as_T4_v3`** (1× NVIDIA T4 16 GB + 16 vCPU per data-loading).
    Fallback: `Standard NCADS_A10_v4 Family` (A10) o T4 in West Europe. Spot T4
    ~$0.30-0.50/h → 24h ≈ ~$10 (< A100 ~$15 → margine credito ancora maggiore).
    **Il render F2-T1 parte comunque sulla quota DSv3 approvata, non aspetta la GPU.**
...
| F2-T3 | Training GPU (T4/A10) → L4 | F2 | ⊘ | F2-T1 *(F0-T4c PARTIAL-LOCK 2026-05-24 + F0-T4d LOCKED + Loss Competition 2026-05-25 (B `fp_to_fn_ratio=30`) + Plan A midimap-patch 2026-05-25 ratificati. **A100 DEFINITIVAMENTE NEGATA 2026-05-30** → pivot a `NCASv3_T4` (NC16as_T4_v3), il modello 331k param non richiede A100. Resta gated da F2-T1 (audio Gold) + nuova quota GPU T4/A10)* | **L4** |
...
· **2026-05-27 — F0-T4f Step B (loss redesign Ridnik) LOCKED v1.0.0 (Decision Lock CEO) + training notturno IN CORSO.** Sessione mattina-pomeriggio post-Step-A. Executive Briefing STRP-001 presentato; CEO vota su D1-D4: ✅ **D1** variante **B'-RIDNIK** (Ridnik AsymmetricLoss Alibaba MS-COCO 2020 — γ_pos=1, γ_neg=4, prob_clip_neg=0.05 + label_smoothing=0.05); ✅ **D2** scope isolato (no capacity bump simultaneo); ✅ **D3** 150 epoch fp32 MPS `--no-amp` ~6h notturno; ✅ **D4** pass criterion val F production ≥ 0.20. **Diagnosi causale Step B** (riconciliata con la diagnostica Step A 2026-05-27 mattina, `calibration_sweep.json` su F0-T4e): la rete è **sotto-confident sui TP** (6/8 bus preferiscono temperature T<1.0 sharpening, sigmoid 0.05-0.20 sui veri positivi). La leva `fp_to_fn_ratio` (Loss Competition B → 30 ratificato) è al massimo sicuro; spingerla a 100 (B1) aggraverebbe la sotto-confidence. La cura è asimmetrizzare il focusing sul ramo positive (γ+ < γ-) — esattamente quello che fa Ridnik 2020. **Implementazione completa nella stessa sessione $0 Azure:** `LossConfig` esteso con `kind="ridnik"` + `gamma_pos`/`gamma_neg`/`prob_clip_negative`/`label_smoothing` (validazione fail-loud + backcompat AFL e Tversky preservata); `_ridnik_asymmetric_loss` (50 LOC nella `src/neural/loss.py`); preset `--loss-preset ridnik` in `tools/mini_l3_train.py` + report HP fields estesi; spec `F0-T4f_STEP_B_LOSS_REDESIGN_SPEC.md` LOCKED v1.0.0; amendment a `F0-T4a §6.2` (path `kind="ridnik"` + supersede `fp_to_fn_ratio` 3→30) + `DOSSIER §6.2`. **Oracoli §6.3 verdi: 33 nuovi** (22 unit `test_loss_ridnik` validation/math/integration + 4 property Hypothesis non-negativity/perm-invariance/monotonia/determinism + 7 backcompat). **Suite F0: 637 passed, 7 skipped, 1 failed** (pre-esistente `test_bootstrap_ci_mean_is_sample_mean` ε=3e-17 commit `6e5db70`). `ruff` clean + `mypy --strict` parità con baseline. **Training notturno IN CORSO**: `mini-l3-F0T4f-stepB-2026-05-27`, 577 sample baseline-only mixed-5kit, val 115 ShittyKit, MPS fp32 — epoch 1 train_loss = 1.28 (vs F0-T4e baseline 2.41 a epoch 10: la pressione asimmetrica Ridnik converge subito più aggressivo). Risultato atteso domattina; verdict a valle. Costo Azure: **$0**.

· **2026-05-28 — F0-T4f Step B (Ridnik) run #1 FAIL ma CONFRONTO INVALIDO + run isolato C=64 IN CORSO.** Il training notturno `mini-l3-F0T4f-stepB-2026-05-27` ha completato 150/150 epoch (13829 s, final train loss **0.787** = minimo storico) ma val F = **0.099 trainer-tuned / 0.155 calibrato** (Step A su questo checkpoint, lift +120 %). Pass criterion D4 (≥ 0.20) FAIL. **MA**: ispezione checkpoint pre-rilancio ha rivelato un **errore di esecuzione** (Strategic Advisor) — il run NON era "Ridnik isolato" come da spec D2: ha usato i **default del trainer** (C=32, audio_aug OFF, cosine LR OFF, early-stop 30) invece di replicare F0-T4e (C=**64**, audio_aug ON, cosine LR ON, early-stop 20). 4 assi cambiati in direzioni opposte → il numero NON è un A/B loss-vs-loss valido. **La conclusione "Ridnik fallisce" è infondata**: a metà capacità + meno regolarizzazione Ridnik pareggia comunque F0-T4e (0.155 vs 0.149 calibrato). Diagnostica dal `calibration_sweep`: 6/8 bus preferiscono ancora T<1 → la sotto-confidence non risolta dal cambio loss (ma confronto inquinato). **Decision Lock CEO 2026-05-28**: rilanciare l'A/B pulito — `mini-l3-F0T4f-c64-isolated-2026-05-28`, C=64 + audio_aug + cosine LR + early-stop 20 (tutto come F0-T4e) cambiando SOLO la loss → ridnik. 331 225 param, MPS fp32 --no-amp, ~6-7h. **IN CORSO.** Se batte 0.149 calibrato → Ridnik vince; altrimenti R&D loss chiuso. Lezione registrata nel ledger: replicare SEMPRE la baseline coi flag espliciti, mai affidarsi ai default per un A/B. Costo Azure: **$0**.

· **2026-05-28 — F0-T4f Step B A/B PULITO completato → Ridnik WIN MODESTO (+9.4 % production) ma mini-L3 SATURO. R&D loss CHIUSO.** Il run isolato `mini-l3-F0T4f-c64-isolated-2026-05-28` (C=64 + audio_aug + cosine LR + early-stop 20, replica esatta di F0-T4e, **unica variabile = loss ridnik**) ha completato 150/150 epoch (19809 s ≈ 5h30). **Verdetto A/B pulito loss-vs-loss:** production calibrato (Step A, metrica realistica del plugin) Ridnik **0.163** vs loss B **0.149** = **+9.4 %** (Ridnik vince); trainer-tuned (metrica ottimistica) Ridnik 0.173 vs B 0.189 = −8.5 %; F_max 0.644 ≈ 0.640 (pari). Il guadagno di Ridnik è specificamente nell'interazione con la calibrazione a soglia fissa, non in discriminabilità grezza. **Ipotesi centrale Step A FALSIFICATA:** 6/8 bus preferiscono ancora T<1 anche a C=64 → la sotto-confidence è **strutturale** (task/capacità), NON curabile dal lato loss. **D4 (≥ 0.20) FAIL** (0.163). **Conclusione strategica (Decision Lock CEO):** il mini-L3 single-kit-val ShittyKit è **saturo** come strumento diagnostico — nessuna leva (AFL, Tversky, 7 candidati Loss Competition, Ridnik, C=32/64/128, P1/P2, audio_aug, input-agnostic, Plan A midimap) rompe oltre ~0.16-0.19 production su un val a kit singolo estremamente OOD. Ridnik resta un win netto modesto a costo zero → **candidabile come default loss per F2-T3**. Il gate reale di validazione si sposta a **L4 / E-GMD** (~30 kit reali, distribuzione molto meno OOD del singolo ShittyKit). **F2-T3 architettonicamente pronto** (F0-T4a + T4c + T4d + loss B/ridnik + Plan A midimap + T4e tutti ratificati). Costo Azure: **$0**.

· **2026-05-28 — PIPELINE AUDIT completo + F0-T18 MIDI Standard Translation Layer LOCKED (Decision Lock CEO). Bug MAJOR risolto: hihat edge GM 22/26 droppato (10 % degli onset).** Audit dataset richiesto dal CEO post-Step-B (`docs/gates/PIPELINE_AUDIT_2026-05-28.md`). **Finding di testa:** la GMD usa **Roland TD-11**, non General MIDI; le note hihat "edge" (GM 22/26) e hi-floor (58) cadevano fuori da OGNI consumatore (mapping table + midimap DrumGizmo + SFZ) e venivano droppate **in silenzio** — **10,1 % di TUTTI gli onset, 34 % di tutto l'hihat**, su 50,7 % dei file GMD. Spiega causalmente il floor persistente dell'hihat in tutto il mini-L3 della settimana (l'hihat reale aveva 1/3 di colpi in meno nel Gold). Altri finding: peak audio >1.0 su 8 % dei render DG (MEDIUM, contratto F0-T2a), GM 58 mismatch su 2 kit (MINOR). Verificati sani: allineamento audio↔target (0 mismatch), NaN/inf (0), canali-zero (caso SFZ noto, gestito da F0-T4e), crash_a rarità legittima. **Correzione al record:** il "Bug #3" dell'handoff (GM 75/54 splash/tambourine) era impreciso — splash(55) è mappato, 54/75 non esistono nella GMD; il vero bug era 22/26, un ordine di grandezza più grande. **Direttiva CEO:** «una volta per tutte coerenza nelle traduzioni tra standard, mai più drop silenziosi» → soluzione strutturale STRP-001 (D1+D2+D3 ratificati): SSoT `midi_source_standards.yaml` (15 articolazioni canoniche + gm_standard + roland_td11 + registro `ignored` esplicito); `midi_canonical.py` con `canonicalize_midi` **fail-loud** sui drop silenziosi (principio cardine); canonicalizzazione **una volta pre-split** in `orchestrate` (render + last_onset + target dallo stesso MIDI canonico → impossibile divergere); `recipe.midi_source.standard`; coherence validator `tools/audit_midi_coherence.py` come **gate pre-render** (0 BLOCKER su GMD, 4 WARN espliciti = limitazioni fisiche kit). **+12 oracoli, suite F0 649 passed, ruff + mypy puliti.** **Prova empirica end-to-end:** re-render mini-L3 canonicalizzato (916 DG + 234 SFZ train, 115 val; backup pre-bug in `data/gold/_prebug_backup_2026-05-28/`) → hihat onset_frames **val 721→1089 (+51 %), train 542→961 (+77 %)**, combaciante al punto con la predizione teorica (+51 %); hihat pos_weight 200→112 (densità raddoppiata). Training A/B `mini-l3-canon-F0T18-2026-05-28` (config esatta F0-T4e, unica variabile = Gold canonico) **IN CORSO** → misura il lift F, soprattutto per-bus hihat, vs baseline F0-T4e 0.149/0.189. Commit `<F0-T18>`. **Sblocca F2-T1 in sicurezza** (il render 1.5 TB ora canonicalizza → niente hihat sventrato a scala 1000×). Costo Azure: **$0**.

· **2026-05-29 — F0-T19 Fase B chunks 1+2: migrazione atomica `flat-25 → flat-28` COMPLETA & verde.** Realizzato il contratto d'uscita LOCKED di F0-T19 §7b (Arrow ③): `N_BUSES → N_CHANNELS = 9`, `TARGET_COLS 25 → 28`, `HIHAT_OPENING_COL 24 → 27`. Le 9 classi-tipo: `kick · snare_head · snare_sidestick · hihat · tom · ride_bow · ride_bell · crash · aux` (sidestick e ride_bell separati come canali; tom/crash collassati per identità; china+splash → aux). Migrazione atomica in singolo run (nessuno stato verde intermedio possibile): 2 SSoT YAML (`midi_mapping_table.yaml` schema **2.0** GM→9-canali + reverse; `midi_source_standards.yaml` bus allineati), ~15 moduli src (`gold_writer`, `target_builder`, `midi_canonical`, `dna_trace`, `neural/{model,loss,metrics,data,export,reporter,train}`, `evaluation/{data_audit,split_consistency,evaluation_suite}`), 4 tool (`scan_density`, `mini_l3_train`, `calibration_sweep`, `listening_test`), ~22 file di test + nuovo oracolo CC#4. **Testa di apertura Hi-Hat da CC#4 continuo** (`openness = clamp(1 − cc4/127)`) — direzione decisa **empiricamente** (probe GMD 2026-05-29: 100 % dei file ha CC#4; closed↔pedale alto mediana 90, open↔pedale basso mediana 17 → CC#4 = pressione pedale), con fallback alle note discrete dove CC#4 assente (~9 %). **Suite 645 passed, 1 failed** (il fail è il pre-esistente `test_bootstrap_ci_mean_is_sample_mean` ε≈3e-17, commit `6e5db70`, estraneo a F0-T19). `ruff` net −1 vs HEAD (zero nuovi; HEAD `c4c4b79` aveva già 22 errori pre-esistenti in file non toccati), `mypy --strict` identici 3 errori pre-esistenti (verificato a HEAD via git stash) → **zero errori nuovi introdotti**. Ocular Proof: 9 acceptance Sfizz render → flat-28; smoke neurale `model→[B,T,28]` + loss ridnik 9-canali finita + `evaluate_l3`. ⏳ **chunks 3+4 (re-render dialect+flat-28 → scan_density 9-tuple → re-train ~6 h config-esatta-F0T4e loss B → verdetto vs baseline 0.149 production / 0.189 trainer-tuned)** NON lanciati a fine sessione per evitare un A/B invalido da config affrettata (lezione ledger F0-T4f run #1): comandi esatti nell'handoff `session-handoff-2026-05-29-f0t19-flat28`. Tutto uncommitted su `develop`. Costo Azure: **$0**.

· **2026-05-29 — F0-T19 chunks 3+4 + verdetto re-train + Fase A chiusa.** (1) **CC#4 fix**: scoperto che `apply_midi_jitter` scartava i `control_change` → la testa di apertura Hi-Hat CC#4 era inerte (fallback discreto su tutto il Gold). Fix in `jitter.py` (preserva tutti i messaggi non-nota), +1 oracolo, re-render → CC#4 continuo confermato (31/40 sample). (2) **Re-render mini-L3** dialect+flat-28+CC#4: 1150 train + 115 val, 0 fail; `scan_density` 9-tuple. (3) **Fase A**: ritirate le 6 patch Plan-A (`*.orig` ripristinati, git-ignored, dead-code perché orchestrate genera i midimap dal dialect map); catturato e corretto un miss della migrazione flat-28 (`audit_midi_coherence` aveva `bus ≤ 8` hardcoded → BLOCKER spuri su china/splash bus 9; corretto a `N_CHANNELS` → gate PASS, 8 WARN dead-code attesi). (4) **Re-train A/B** (`mini-l3-F0T19-flat28-2026-05-29`, config esatta F0-T4e, unica var = Gold flat-28): early-stop ep111, 4.1h. **VERDETTO: production-calibrato 0.1455 vs baseline 0.149 → FLAT** (−2.3 %); trainer-tuned 0.257 vs 0.189 (+36 %, confuso dalla media su 8→9 canali); **F_max 0.763 vs 0.640 = nuovo picco cross-kit**. **Perché flat**: lo split flat-28 a 9 classi crea 4 canali rari (snare_sidestick/ride_bell/crash/aux) **quasi vuoti sul val singolo ShittyKit** (F per-bus ≈ 0) che diluiscono la media — in flat-25 erano collassati su canali-con-segnale; la diluizione **si inverte su un val multi-kit**. **Conclusione**: contratto F0-T19 VALIDATO (training pulito, nessuna regressione, tassonomia corretta + Gold più pulito), headline al **floor saturo ~0.15** già diagnosticato da F0-T4f. **Architettonicamente GO per F2-T3**; gate reale = L4/E-GMD (~30 kit, molto meno OOD). Suite 645 passed (+ nuovi oracoli CC#4/flat-28), ruff/mypy parità, $0 Azure, tutto uncommitted su `develop`.

· **2026-05-30 — Audit readiness F2-T1 + F0-T20 Franken-Kit STRP-001 LOCKED (sessione "caffeina").** (1) **Audit F2-T1** (`docs/gates/PIPELINE_AUDIT_F2T1_READINESS_2026-05-30.md`): gli artefatti Azure (`build_recipe_matrix.py`, `provision_render_vm.sh`, runbook — tutti del 23-05) **precedono F0-T18/T19** → **F2-T1 NON era render-ready**. 3 BLOCKER: B1 canonicalizzazione saltata (recipe senza `standard` → `orchestrate:406` la gata → bug hihat GM 22/26 a scala), B2 5/10 kit fuori dal dialect map, B3 path/nomi stale. **B1+M5 FIXED** (commit `2b44639`: `standard: roland_td11` + `bus_mapping@2.0`, smoke verde). B2/B3 → keymap SFZ reali letti da GitHub (no WAV, idea CEO): **Frankensnare = solo-snare (key=37)**, **VSCO2CE = percussione orchestrale (è un saboteur, non un kit)** → inadatti; **UnrulyDrums/SwirlyDrums GM-keyed** → legacy ok. Roster 8-kit ratificato CEO (B3 applicato dopo F0-T20). (2) **F0-T20 Cross-Kit Combinatorial Augmentation (Franken-Kit) STRP-001 LOCKED v1.0.0** (`docs/methodology/F0-T20_CROSS_KIT_AUGMENTATION_SPEC.md`): ogni campione = kit ibrido, ogni strumento da un kit diverso → attacca il fingerprint **timbrico** (ciò che F0-T4e non poteva, solo layout) = movente del floor saturo ~0.15. Tecnica pubblicata/validata (**StemGMD/LarsNet** PoliMI: stessa GMD, stesso 9-piece canonico, generalizza a kit held-out). Decision Lock CEO D1-D8: **D1** Approccio A (render per-strumento + somma 8-bus), **D2** within-engine v1.0, **D3** 50% franken + 50% single-kit, **D5** keep both (stem esatti franken + Demucs su single-kit reali), **D8** Frankensnare riammesso come sorgente-rullante. Side-win: stem ground-truth esatti + R&D source-separation. Implementazione = sotto-task **F0-T20b** `[F]`. **Cambia il render → gate di F2-T1.** Costo Azure: $0 (sviluppo locale; a F2-T1 ×N render dentro budget con cap frazione D3).

· **2026-05-31 — F0-T20b Franken-Kit implementato + A/B capacità (sessione autonoma overnight, $0 Azure).** Implementazione completa del Cross-Kit Combinatorial Augmentation (`src/data_engineering/gold/franken.py`: split MIDI per-strumento + assegnazione kit sha256 + render-per-famiglia-e-somma; 14 oracoli; `tools/franken_smoke.py` + `tools/build_franken_dataset.py`). Dataset franken 496 train (230 hybrid + 266 single, 50/50 = D3, ×2 jitter, 4 kit DG within-engine) / 248 ShittyKit val. **A/B production-calibrato vs floor single-kit 0.1455:** franken **C=64 = 0.1599 (+9.9 %, MIGLIORE)** — primo lever-dati che muove il floor saturo (F0-T19 sintetico e F0-T4f loss erano flat); franken **C=128 (4× params) = 0.1531** → la capacità NON aiuta il production (alza solo il trainer-tuned 0.165→0.230 = overfit soglia per-sample). **Decisioni operative**: (1) franken-kit **ratificato** come augmentation → **includere in F2-T1**; (2) **restare a C=64** per F2-T3 (C=128 non ripaga). Il val single-kit ShittyKit è saturo ~0.16 su ogni lever (capacità/loss/aug/franken) → gate reale = L4/E-GMD (~30 kit). Commit `fca4b6b`→`2e34831`. Report `docs/gates/F0-T20b_FRANKEN_CAPACITY_AB_2026-05-31.md`; ledger run 26+27.
